...
parser.add_argument('--seed', type=int, nargs='+', default=[111],
                    help='numpy random seeds, e.g. --seed 111 222 333')                
...
mEER_L = []; mEER_Q = []
dataset_name = "MCYT" 
model_dataset_name = "%s_%s" % (modelname, dataset_name)
print("For MCYT:")
for seed in seeds:
    save_path = "log/seed%d/%s" % (seed, model_dataset_name)
    DIST_P = numpy.load("%s/dtw_dist_p%s.npy" % (save_path, args.epoch))
    DIST_N = numpy.load("%s/dtw_dist_n%s.npy" % (save_path, args.epoch))
    DIST_TEMP = numpy.load("%s/dtw_dist_temp%s.npy" % (save_path, args.epoch))
    if LOOP:
        print('loop=',LOOP)
        for i in range(4):
            datum_p = []
...
mEER_Q.append(numpy.mean(EER_G))
...
for seed in seeds:
    save_path = "log/seed%d/%s" % (seed, model_dataset_name)
    DIST_P = numpy.load("%s/dtw_dist_p%s_d0.npy" % (save_path, args.epoch))
    DIST_N = numpy.load("%s/dtw_dist_n%s_d0.npy" % (save_path, args.epoch))
    DIST_TEMP = numpy.load("%s/dtw_dist_temp%s_d0.npy" % (save_path, args.epoch))
    if LOOP:
        for i in range(4):
            datum_p = []
            datum_n = []
...
for seed in seeds:
    save_path = "log/seed%d/%s" % (seed, model_dataset_name)
    DIST_P = numpy.load("%s/dtw_dist_p%s_d1.npy" % (save_path, args.epoch))
    DIST_N = numpy.load("%s/dtw_dist_n%s_d1.npy" % (save_path, args.epoch))
    DIST_TEMP = numpy.load("%s/dtw_dist_temp%s_d1.npy" % (save_path, args.epoch))
    if LOOP:
        for i in range(4):
            datum_p = []
            datum_n = []
...
for seed in seeds:
    save_path = "log/seed%d/%s" % (seed, model_dataset_name)
    DIST_P = numpy.load("%s/dtw_dist_p%s_d2.npy" % (save_path, args.epoch))
    DIST_N = numpy.load("%s/dtw_dist_n%s_d2.npy" % (save_path, args.epoch))
    DIST_TEMP = numpy.load("%s/dtw_dist_temp%s_d2.npy" % (save_path, args.epoch))
    if LOOP:
        for i in range(4):
            datum_p = []
            datum_n = []
...
for seed in seeds:
    save_path = "log/seed%d/%s" % (seed, model_dataset_name)
    DIST_P = numpy.load("%s/dtw_dist_p%s_d3.npy" % (save_path, args.epoch))
    DIST_N = numpy.load("%s/dtw_dist_n%s_d3.npy" % (save_path, args.epoch))
    DIST_TEMP = numpy.load("%s/dtw_dist_temp%s_d3.npy" % (save_path, args.epoch))
    if LOOP:
        for i in range(4):
            datum_p = []
            datum_n = []
...
for seed in seeds:
    save_path = "log/seed%d/%s" % (seed, model_dataset_name)
    DIST_P = numpy.load("%s/dtw_dist_p%s_d4.npy" % (save_path, args.epoch))
    DIST_N = numpy.load("%s/dtw_dist_n%s_d4.npy" % (save_path, args.epoch))
    DIST_TEMP = numpy.load("%s/dtw_dist_temp%s_d4.npy" % (save_path, args.epoch))
    if LOOP:
        for i in range(4):
            datum_p = []
            datum_n = []
...
user_all = numpy.array([100, 132, 140, 35, 35, 35, 35, 35, 35])
print("Overall EER under user-specific threshold:",numpy.sum(numpy.array(mEER_L) * user_all) / numpy.sum(user_all))
